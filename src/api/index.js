...
const HttpManager = {
  ...
  addTableTwo:(params) => get(`/Table/add`,params),
  // =======================> 收藏列表 API
  // 返回的指定用户ID收藏列表
...
//删除一条信息
  deleteTableTwoList:(id) => get(`Table/delete?id=${id}`),
  //根据userid查询信息
  TableList2:(id) => get(`/TableList2?id=${id}`),
  // =======================> 歌曲 API
  // 返回所有歌曲
  getAllSong: () => get(`song`),
  // 返回指定歌手ID的歌曲
  ...
  //获取Product表单
  getqueryAllProduct:() =>get(`product/queryAllProduct`),
  //删除一条Product表信息
  deleteProduct:(id) => get(`/product/deleteProduct?product_id=${id}`),
  //根据Product表id查询
  queryProductByProduct_Id:(id) => get(`product/queryProductByProduct_Id?product_id=${id}`),
  //添加Product表单一条表信息
  insertProduct:(params) => get(`/product/insertProduct`,params),
  //修改Product表单一条表信息
  updateProduct:(params) => get(`product/updateProduct`,params),
  //根据Product表多条件查询
  queryProductBy:(params) => get(`product/queryProductBy`,params),
}
